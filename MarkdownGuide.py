			...
			for name, info in infos:
				...
				with open(os.path.join(path, 'README.md'), 'w', encoding = 'utf-8') as single:
					...
					for index, level in enumerate(info['difficulty']):
						...
						upload(release, pez, version)
						upload(release, pez, version, f'{name}-{difficulty}.zip')
						upload(release, phirapez, version)
						upload(release, phirapez, version, f'{name}-{difficulty}(Phira ver.).zip')
						upload(release, rpepez, version)
						upload(release, rpepez, version, f'{name}-{difficulty}(RPE ver.).zip')
						upload(release, origchart, version, f'{name}-{difficulty}.json')
						upload(release, rpechart, version, f'{name}-{difficulty}(RPE ver.).json')
						upload(release, phirachart, version, f'{name}-{difficulty}(Phira ver.).json')
						...
						upload(release, Phichain, version)
						os.remove(Phichain)
						single.write(
							f'''
