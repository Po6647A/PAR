...
for id, info in track(infos.items(), description = "WritingPhichainProject"):
    for level in range(len(info["difficulty"])):
            if os.path.exists(f"Phigros_Resource/Chart_{levels[level]}/{id}.0.phichain.json"):
                try:
                    ...
                    os.rename(baseDir+f'/{id}.ogg', baseDir + "/music.ogg")
                except:
                    try:
                        shutil.rmtree(f"PhichainProject/{levels[level]}/{id}")
                    except:
                        pass